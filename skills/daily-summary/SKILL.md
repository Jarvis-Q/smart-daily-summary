...
3. **生成与保存**：结合数据与用户回复，生成标准化 Markdown 日报，保存至 `~/Documents/WorkLogs/daily-summary-YYYY-MM-DD.md`。
4. **结束提示**：告知用户日报已保存，提示可通过 `/daily-summary:send` 发送，或在编辑器中修改后再发送。

### 分支 2: `/daily-summary:send` (发送最新日报)
1. 自动定位 `~/Documents/WorkLogs/` 目录下最新生成的 `daily-summary-YYYY-MM-DD.md` 文件。
